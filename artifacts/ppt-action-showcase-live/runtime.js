...
const motionProfile = {"highlight":{"enter":{"delayMs":70,"durationMs":260,"easing":"ease-in-out"},"exit":{"delayMs":10,"durationMs":190,"easing":"ease-in"}},"appear":{"enter":{"delayMs":140,"durationMs":460,"easing":"cubic-bezier(0.16, 1, 0.3, 1)"},"exit":{"delayMs":0,"durationMs":180,"easing":"ease-in"}},"spotlight":{"enter":{"delayMs":60,"durationMs":320,"easing":"ease-out"},"exit":{"delayMs":0,"durationMs":180,"easing":"ease-in"}},"laser":{"enter":{"delayMs":190,"durationMs":520,"easing":"cubic-bezier(0.22, 1, 0.36, 1)"},"exit":{"delayMs":0,"durationMs":190,"easing":"ease-in"}},"byBlockKind":{"timeline":{"spotlight":{"enter":{"delayMs":40,"durationMs":280,"easing":"ease-out"},"exit":{"delayMs":0,"durationMs":160,"easing":"ease-in"}},"laser":{"enter":{"delayMs":150,"durationMs":440,"easing":"cubic-bezier(0.22, 1, 0.36, 1)"},"exit":{"delayMs":0,"durationMs":170,"easing":"ease-in"}}},"comparison":{"highlight":{"enter":{"delayMs":90,"durationMs":300,"easing":"ease-in-out"},"exit":{"delayMs":20,"durationMs":210,"easing":"ease-in"}},"appear":{"enter":{"delayMs":170,"durationMs":500,"easing":"cubic-bezier(0.16, 1, 0.3, 1)"},"exit":{"delayMs":10,"durationMs":190,"easing":"ease-in"}}}}};
...
function currentRenderState(actions, guidance) {
  ...
  // focusMode only activates on explicit highlight actions, not appear or guidance
  const focusMode = highlightedIds.size > 0;
  // keep guidance targets available to overlay geometry even when focusMode is off
  const emphasizedIds = new Set([...highlightedIds, ...guidanceTargetIds]);
...
function actionBlockKind(action, context) {
  if (!action) {
    return context.primaryRect?.kind ?? null;
  }

  if (action.kind === "highlight" || action.kind === "appear" || (action.kind === "spotlight" && action.targetId)) {
    const target = context.targetRects?.find((rect) => rect.id === action.targetId);
    return target?.kind ?? context.primaryRect?.kind ?? null;
  }

  return context.primaryRect?.kind ?? context.targetRects?.[0]?.kind ?? null;
}
...
function renderStageOverlay(actions, renderState, exitSnapshot) {
  ...
  if (exitSnapshot && exitSnapshot.slideIndex === state.slideIndex) {
    const exitRects = [];
    ...
    const exitPrimaryRect = exitRects.find((rect) => exitSnapshot.renderState.highlightedIds.has(rect.id))
      ?? exitRects.find((rect) => exitSnapshot.renderState.guidanceTargetIds.has(rect.id))
      ?? exitRects[0]
      ?? null;
...
function renderOverlay(actions, context, options = {}) {
  ...
  const spotlightTargetRect = findStageRectForBlockId(spotlight?.targetId, targetRects)
    ?? targetRects.find((rect) => guidanceTargetIds.has(rect.id))
    ?? primaryRect
    ?? targetRects[0]
    ?? null;
  ...
  for (const laser of lasers) {
    const lt = resolveActionTiming(laser, "laser", phase,
      { delayMs: 210, durationMs: 620, easing: phase === "exit" ? "ease-in" : "cubic-bezier(0.22, 1, 0.36, 1)" },
      actionBlockKind(laser, context));
    const pts = laser.points.map((p) => stagePoint(p));
    if (primaryRect) {
      pts.unshift({ x: primaryRect.x + primaryRect.width / 2, y: primaryRect.y + primaryRect.height / 2 });
    }
    const pointStr = pts.map((p) => p.x + "," + p.y).join(" ");
    const last = pts[pts.length - 1];

    // Trail dots behind the laser dot
    const trailCount = Math.min(pts.length - 1, 3);
    for (let i = 1; i <= trailCount; i++) {
      const tp = pts[pts.length - 1 - i];
      if (!tp) break;
      const tDelay = lt.delayMs + 100 + i * 80;
      els.push(svg("circle", {
        class: "action-laser-trail", cx: tp.x, cy: tp.y, r: 5,
        style: "--action-delay:" + tDelay + "ms"
      }));
    }

    els.push(svg("polyline", {
      class: "action-laser-line", points: pointStr,
      filter: "url(#glow-strong-" + instanceId + ")",
      style: overlayStyle(lt)
    }));
    els.push(svg("circle", {
      class: "action-laser-dot", cx: last.x, cy: last.y, r: 7,
      filter: "url(#glow-" + instanceId + ")",
      style: overlayStyle(lt, lt.delayMs + 120)
    }));
  }
