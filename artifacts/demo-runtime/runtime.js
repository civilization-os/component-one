...
function renderStageOverlay(actions, renderState, exitSnapshot) {
  ...
  const overlays = [];
  const activeHasSpatialAction = actions.some((action) => action.kind === "laser" || action.kind === "spotlight");

  if (exitSnapshot && exitSnapshot.slideIndex === state.slideIndex && !activeHasSpatialAction) {
    const exitRects = [];
...
function renderOverlay(actions, context, options = {}) {
  ...
  const shouldRenderGuidanceLines = lasers.length === 0 && !spotlight;
  ...
  for (const laser of lasers) {
    ...
    // Position: if paired with highlight, dot lands near the highlighted block.
    // Otherwise, use the laser action's first point.
    let cx, cy;
    if (highlightAction && primaryRect) {
      cx = primaryRect.x + primaryRect.width / 2;
      cy = primaryRect.y + primaryRect.height / 2;
      // Offset slightly below center so the dot doesn't sit right on the focus ring
      cy = Math.min(STAGE_HEIGHT, cy + primaryRect.height * 0.18);
    } else {
